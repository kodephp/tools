...
### v1.1.3 (2025-12-26)
- ✅ 优化数组处理模块，支持PHP 8.4+原生数组函数
- ✅ 添加字符串处理模块新方法（truncate、limit、snake、camel等）
- ✅ 完善全局助手函数，支持新的数组和字符串方法
- ✅ 更新项目规则文件，添加详细设计文档
- ✅ 完善README文档，添加详细使用示例
...
- ✅ 初始版本发布
- ✅ 实现消息体模块（链式调用、自定义状态码）
- ✅ 实现加解密模块（MD5、密码哈希、SSL加密、HMAC签名）
- ✅ 实现代码生成模块（订单号、邀请码、URL安全码、注册码）
- ✅ 实现数组处理模块（树形结构、深度合并、多维数组统计）
- ✅ 实现字符串处理模块（脱敏、验证、转换、编码解码）
- ✅ 实现时间处理模块（格式化、计算、日期范围）
- ✅ 实现数学计算模块（高精度计算、金融计算、统计分析）
- ✅ 实现地理位置模块（距离计算、坐标验证、中点计算）
- ✅ 实现IP地址处理模块（IP获取、验证、私有IP检测）
- ✅ 实现全局助手函数（简化调用方式）
...
#### <a name="distance"></a> `distance(float $lat1, float $lon1, float $lat2, float $lon2, string $unit = 'km')`
计算两点之间的距离
...
#### <a name="isValidCoordinate"></a> `isValidCoordinate(float $lat, float $lon)`
验证坐标是否有效
...
#### <a name="wgs84ToGcj02"></a> `wgs84ToGcj02(float $lat, float $lon)`
WGS84转GCJ02（火星坐标系）
...
#### <a name="gcj02ToBd09"></a> `gcj02ToBd09(float $lat, float $lon)`
GCJ02转BD09（百度坐标系）
...
#### <a name="getBearing"></a> `getBearing(float $lat1, float $lon1, float $lat2, float $lon2)`
计算两点之间的方位角
...
#### <a name="getMidpoint"></a> `getMidpoint(float $lat1, float $lon1, float $lat2, float $lon2)`
计算两点之间的中点坐标
